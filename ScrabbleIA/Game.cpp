...
void	Game::init(CliIRC& client)
{
	...
	client.addEvent("botnotice", &Game::oth_msg);
...
void	Game::bot_join(CliIRC& cli, void *ptr, UserIRC *src, ChannelIRC *chan, UserIRC *dst, std::string msg)
{
	if (cli.configIsOnList("AUTOJOIN", chan->getName()) == "-1")
		...
		cli.say(chan->getName(), "!go");
	chan->_game._scrabble_word = "";
...
void	Game::oth_join(CliIRC& cli, void *ptr, UserIRC *src, ChannelIRC *chan, UserIRC *dst, std::string msg)
{
	...
	if (chan->_game._state == 2)
	{
		...
		cli.notice(src->getNick(), cli.lang("GAME", "join_started_2", GAME_NAME, (*(chan->_game._turn)).user->getNick().c_str()));
	}
...
void				Game::oth_msg(CliIRC& cli, void *ptr, UserIRC *src, ChannelIRC *chan, UserIRC *dst, std::string msg)
{
	...
	static std::list<char>																				letters;
	std::string																							command;
	std::string																							parameters;
	int																									i;

	if (chan == NULL)
		chan = *(cli._channels.begin());
	msg = lowCase(cli.strip(msg));
	if (gettok(msg, 1, ' ') == "bonjour")
	  if (gettok(msg, 2, ' ') == "tout")
	    if (gettok(msg, 3, ' ') == "le")
	      if (gettok(msg, 4, ' ') == "monde")
		if (gettok(msg, 5, ' ') == "!")
		  cli.say(chan->getName(), "!go");
	if (gettok(msg, 1, ' ') == "il")
		if (gettok(msg, 2, ' ') == "y")
			if (gettok(msg, 3, ' ') == "a")
				if (gettok(msg, 4, ' ') == "1")
					if (gettok(msg, 5, ' ') == "joueur")
						if (gettok(msg, 6, ' ') == "inscrit:")
							cli.say(chan->getName(), "!go");
	if (gettok(msg, 1, ' ') == "le")
		if (gettok(msg, 2, ' ') == "mot")
			if (gettok(msg, 3, ' ') == "actuel")
				if (gettok(msg, 4, ' ') == "est:")
					chan->_game._scrabble_word = gettok(msg, 5, ' ');
	if (gettok(msg, 1, ' ') == "tes")
		if (gettok(msg, 2, ' ') == "lettres")
			if (gettok(msg, 3, ' ') == "sont:")
			{
				letters.clear();
				for (i = 4; (i <= numtok(msg, ' ')); i += 2)
					letters.push_back(gettok(msg, i, ' ').c_str()[0]);
			}
	if (gettok(msg, 1, ' ') == "c'est")
		if (gettok(msg, 2, ' ') == "a")
			if (gettok(msg, 3, ' ') == lowCase(cli.me()))
				if (gettok(msg, 4, ' ') == "de")
					if (gettok(msg, 5, ' ') == "jouer:")
					{
						map = chan->_game.scrabble_ia(letters);
						if (map.size() > 0)
							cli.say(chan->getName(), "!jeu " + chan->_game.scrabble_colorize_word((*(map.begin())).word));
						else
							cli.say(chan->getName(), "!p");
					}
	if (gettok(msg, 1, ' ') == "la")
		if (gettok(msg, 2, ' ') == "partie")
			if (gettok(msg, 3, ' ') == "est")
				if (gettok(msg, 4, ' ') == "terminee,")
					if (gettok(msg, 5, ' ') == "au")
						if (gettok(msg, 6, ' ') == "bout")
						{
							chan->_game._scrabble_word = "";
							cli.say(chan->getName(), "!go");
						}
						ptr = ptr; dst = dst; src = src;
	return ;
}
...
void								Game::_game_score(CliIRC& client, ChannelIRC *chan, UserIRC *user, std::string& msg)
{
	...
	{
		for (it = this->_players.begin(); (it != this->_players.end()); it++)
			if (lowCase((*it).user->getNick()) == lowCase(nick))
			{
				if ((*it).score == 0)
...
void		Game::_play_new_turn(CliIRC& client, ChannelIRC *chan)
{
	...
	client.say(chan->getName(), client.lang("MODULE", "new_turn", (*(this->_turn)).user->getNick().c_str()));
	client.say(chan->getName(), client.lang("GAME", "line"));
...
void			Game::_play_timeout_c1(void *cli, void *chan, void *nul)
{
	...
	client->say(channel->getName(), client->lang("GAME", "timeout_c1", (*(channel->_game._turn)).user->getNick().c_str()));
	nul = nul;
	return ;
}
...
void		Game::_play_timeout_c2(void *cli, void *chan, void *nul)
{
	...
	client = (CliIRC *)cli;
	channel = (ChannelIRC *)chan;
	client->say(channel->getName(), client->lang("GAME", "timeout_c2", (*(channel->_game._turn)).user->getNick().c_str()));
	nul = nul;
	return ;
}
...
void		Game::_play_timeout_c3(void *cli, void *chan, void *nul)
{
	...
	client = (CliIRC *)cli;
	channel = (ChannelIRC *)chan;
	client->say(channel->getName(), client->lang("GAME", "timeout_c3", (*(channel->_game._turn)).user->getNick().c_str()));
...
bool								Game::_play_is_turn_player(CliIRC& client, UserIRC *user)
{
	...
	if (it == this->_players.end())
		...
		client.notice(user->getNick(), client.lang("GAME", "not_your_turn", (*(this->_turn)).user->getNick().c_str()));
	else
		return (true);
	return (false);
}
...
void								Game::_play_winner(CliIRC& client, ChannelIRC *chan)
{
	...
	client.say(chan->getName(), client.lang("GAME", "line"));
	if ((*it).finish == 1)
		client.say(chan->getName(), client.lang("GAME", "finish_1", (*it).user->getNick().c_str()));
	else
		client.say(chan->getName(), client.lang("GAME", "finish_n", (*it).user->getNick().c_str(), intToString((*it).finish).c_str()));
...
void														Game::_play_finish(CliIRC& client, ChannelIRC *chan)
{
	...
	{
		if ((*itp).score == -1)
			client.say(chan->getName(), client.lang("GAME", "play_top_abandon", intToString(num).c_str(), (*itp).user->getNick().c_str(), intToString((*itp).nb_secs / 60).c_str(), intToString((*itp).nb_secs % 60).c_str()));
		else
			client.say(chan->getName(), client.lang("GAME", "play_top_finish", intToString(num).c_str(), (*itp).user->getNick().c_str(), intToString((*itp).score).c_str(), intToString((*itp).nb_turn).c_str(), intToString((*itp).nb_secs / 60).c_str(), intToString((*itp).nb_secs % 60).c_str()));
		num++;
	}
	client.say(chan->getName(), client.lang("GAME", "line"));
	for (itp = this->_players.begin(); (itp != this->_players.end()); itp++)
		if ((*itp).score > 0)
		{
			if (client.configIsKey("SCORE", lowCase((*itp).user->getNick())) == false)
			{
				num = 0;
				nbr = 0;
			}
			...
			{
				num = ::atoi(gettok(((*client._config)["SCORE"][lowCase((*itp).user->getNick())]), 1, ':').c_str());
				nbr = ::atoi(gettok(((*client._config)["SCORE"][lowCase((*itp).user->getNick())]), 2, ':').c_str());
			}
			num += (*itp).score;
			nbr += 1;
			(*client._config)["SCORE"][lowCase((*itp).user->getNick())] = intToString(num) + ':' + intToString(nbr);
		}
	client.configSave();
